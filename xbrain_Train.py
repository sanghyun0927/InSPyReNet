...
from submodules.InSPyReNet.lib import *
from submodules.InSPyReNet.lib.optim import *
from submodules.InSPyReNet.data.dataloader import *
from submodules.InSPyReNet.utils.misc import *
...
def train(opt, args):
    ...
    model_ckpt = torch.load('../../submodules/InSPyReNet/pretrained_ckpt/ckpt_bash.pth')
    state_ckpt = None

    if args.resume is True:
        if os.path.isfile(os.path.join(opt.Train.Checkpoint.colob_dir, 'latest.pth')):
            model_ckpt = torch.load(os.path.join(opt.Train.Checkpoint.colob_dir, 'latest.pth'), map_location='cpu')
            if args.local_rank <= 0:
                print('Resume from checkpoint')
        if os.path.isfile(os.path.join(opt.Train.Checkpoint.colob_dir, 'state.pth')):
            state_ckpt = torch.load(os.path.join(opt.Train.Checkpoint.colob_dir, 'state.pth'), map_location='cpu')
            if args.local_rank <= 0:
    ...
    for epoch in epoch_iter:
        ...
        if args.local_rank <= 0:
            os.makedirs(opt.Train.Checkpoint.colob_dir, exist_ok=True)
            os.makedirs(os.path.join(
                opt.Train.Checkpoint.colob_dir, 'debug'), exist_ok=True)
            if epoch % opt.Train.Checkpoint.checkpoint_epoch == 0:
                ...
                torch.save(model_ckpt, os.path.join(opt.Train.Checkpoint.colob_dir, 'latest.pth'))
                torch.save(state_ckpt, os.path.join(opt.Train.Checkpoint.colob_dir, 'state.pth'))

            if args.debug is True:
                debout = debug_tile(sum([out[k] for k in opt.Train.Debug.keys], []), activation=torch.sigmoid)
                cv2.imwrite(os.path.join(opt.Train.Checkpoint.colob_dir, 'debug', str(epoch) + '.png'), debout)

    if args.local_rank <= 0:
        torch.save(model.module.state_dict() if args.device_num > 1 else model.state_dict(),
                   os.path.join(opt.Train.Checkpoint.colob_dir, 'latest.pth'))
